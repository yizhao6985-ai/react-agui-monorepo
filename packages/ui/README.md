...
基于 **Tailwind CSS** 的 AGUI React 组件库，依赖 [react-agui-core](../core/README.md)。提供 `Conversations`、`Bubble`、`Bubble.List`、`Sender` 等带样式的组件。
...
<AGUIProvider url="/agui" storage={createLocalSessionStorage()}>
  ...
  />
  <Bubble.List messages={messages} loading={loading} onEditMessage={editMessage} />
...
| `Bubble.List` | `messages`、`loading?`、`onEditMessage?`、`loadingText?` | 消息列表，含角色标签、loading 状态 |
